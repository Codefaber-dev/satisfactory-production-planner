<template>
    <div @click="select" class="flex-1 flex">
        <div v-show="!slim" class="flex flex-col items-center justify-center">
            <div class="h-20 w-20 dark:text-slate-800 rounded-full bg-blue-200 border border-blue-500 text-xl mx-4 flex flex-col items-center justify-center mb-2">
                {{ +recipe.base_per_min }}
                <span class="text-xs whitespace-nowrap">per min</span>
            </div>
            <div class="h-20 w-20 dark:text-slate-800 rounded-full bg-rose-200 border border-rose-500 text-xl mx-4 flex flex-col items-center justify-center">
        ...
        <div class="flex flex-col">
            ...
            <div class="font-semibold text-lg flex">
                ...
                <div :class="slim ? ['p-1'] : ['px-4','py-2']" v-if="recipe.alt_recipe" class="text-xs dark:text-slate-800 rounded bg-purple-200 border border-purple-500 ml-2">
                    Alt
                </div>
                <div :class="slim ? ['p-1'] : ['px-4','py-2']" v-if="recipe.energy_efficient"
                     class="text-xs dark:text-slate-800 rounded bg-green-200 border border-green-500 ml-2">
                    Energy
                </div>
            ...
            <div class="text-sm">
                <div v-if="recipe.ingredients && recipe.ingredients.length" class="bg-yellow-200 dark:text-slate-800 border-l-4 border-yellow-500 p-2 my-2 rounded flex flex-col">
                    <div v-for="o in recipe.ingredients.map(o => `${o.name} (${+o.pivot.base_qty} per min)`)">{{ o }}</div>
                </div>
                <div class="bg-teal-200 dark:text-slate-800 border-l-4 border-teal-500 p-2 my-2 rounded" v-if="recipe.byproducts && recipe.byproducts.length">
