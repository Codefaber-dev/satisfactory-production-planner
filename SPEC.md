...
| T90 | .      | Building cost multiplier: add `building_cost_multiplier` plan param (float, default 1.0, clamp 0.1–10.0); thread through `ProductionCalculator` → `BuildingDetails::calc` (mirror `cost_multiplier` path via `ProductionGlobals`); apply to `build_cost` maps in both normal + recompute blocks; persist via existing save/fetch flow; UI numeric input beside cost_multiplier; tests: build_cost scales, ingredient consumption/belt load/power/counts unchanged, disjointness from cost_multiplier | I.plan-params, V48 |
## §B — Bug Log
